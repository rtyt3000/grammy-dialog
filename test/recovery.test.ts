...
describe("side-effect recovery", () => {
  test("deletes an orphan surface when initial instance persistence fails", async () => {
    ...
    await expect(user.sendCommand("recovery")).rejects.toThrow("storage write failed");

    expect(chats.outgoing.getMethods()).toContain("sendMessage");
    expect(chats.outgoing.getMethods()).toContain("deleteMessage");
    expect([...storage.readAllKeys()].some(key => key.startsWith("gd:instance:"))).toBe(false);
    expect([...storage.readAllKeys()].some(key => key.startsWith("gd:callback:"))).toBe(false);
  ...
  test("rolls an edited surface back when instance persistence fails", async () => {
    ...
    await expect(reply.clickButton("Increment")).rejects.toThrow("storage write failed");
    ...
    expect(instance?.type === "instance" ? instance.value.state : undefined)
      .toEqual({ count: 0 });
    expect(instance?.type === "instance" ? instance.value.callbackTokens.length : 0).toBe(1);
    const rollbackToken = instance?.type === "instance"
      ? instance.value.callbackTokens[0]
      : undefined;
    expect(rollbackToken === undefined ? undefined : storage.read(`gd:callback:${rollbackToken}`)?.type)
      .toBe("callback");
  });
  ...
  test("keeps visible rollback callbacks when rollback persistence also fails", async () => {
    ...
    await expect(reply.clickButton("Increment"))
      .rejects.toThrow("Failed to roll back Telegram surface");

    const rollbackEdit = chats.outgoing.requests
      .filter(request => request.method === "editMessageText")
      .at(-1);
    const rollbackCallbackData = (rollbackEdit?.payload as {
      reply_markup?: { inline_keyboard?: Array<Array<{ callback_data?: string }>> };
    } | undefined)?.reply_markup?.inline_keyboard?.[0]?.[0]?.callback_data;
  ...
  test("keeps the old surface when replacement persistence fails", async () => {
    ...
    const replaceWindow = window("replacement-recovery", {
      viewModel: vm,
      text: ({ vm }) => vm.withPhoto ? "Photo" : "Text",
      media: ({ vm }) => vm.withPhoto ? photo("replacement-photo") : undefined,
      keyboard: [[button("Attach", "attach")]],
    });
    const storage = new JsonStorageAdapter<DialogStorageRecord>();
    const bot = new Bot<TestContext>("test-token");
    ...
    await expect(oldSurface.clickButton("Attach")).rejects.toThrow("storage write failed");

    const sent = chats.outgoing.requests.find(request => request.method === "sendPhoto");
    const deleted = chats.outgoing.requests.find(request => request.method === "deleteMessage");
    expect(sent).toBeDefined();
    expect((deleted?.payload as { message_id?: number } | undefined)?.message_id)
      .not.toBe(oldSurface.messageId);
    const instance = [...storage.readAllValues()]
      .find(record => record.type === "instance");
    expect(instance?.type === "instance" ? instance.value.surface?.messageId : undefined)
      .toBe(oldSurface.messageId);
    expect(instance?.type === "instance" ? instance.value.state : undefined)
      .toEqual({ withPhoto: false });
    const callbacks = [...storage.readAllKeys()].filter(key => key.startsWith("gd:callback:"));
  ...
  test("keeps a committed replacement when old callback cleanup fails", async () => {
    ...
    const committedToken = instance?.type === "instance"
      ? instance.value.callbackTokens[0]
      : undefined;
    expect(committedToken === undefined ? undefined : storage.read(`gd:callback:${committedToken}`)?.type)
      .toBe("callback");
    expect(chats.outgoing.requests.filter(request => request.method === "deleteMessage"))
      .toHaveLength(1);
  });
});

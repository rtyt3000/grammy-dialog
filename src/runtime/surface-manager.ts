...
export class SurfaceManager<C extends Context = Context, Services = unknown> {
  public constructor(
    ...
    try {
      ...
      if (orphanMessageId !== undefined) {
        await this.ignoreFailure(() =>
          api.deleteMessage(instance.chatId, orphanMessageId),
        );
      }
  ...
  public async rerender(
    ...
    if (instance.surface === undefined)
      throw new Error(`Instance '${instance.id}' has no surface`);
    ...
    try {
      if (operation === "edit") {
        try {
          ...
          instance.surface = this.surface(
            instance,
            previous.surface.messageId,
            rendered,
          );
        } catch (editError) {
          const fallback =
            (await this.presentation.fallbackAfterEditError?.(editError)) ??
            "throw";
      ...
      if (orphanMessageId !== undefined) {
        ...
        await this.ignoreFailure(() =>
          this.repository.deleteCallbacks(rendered.callbackTokens),
        );
    ...
    await this.ignoreFailure(() =>
      this.repository.deleteCallbacks(previous.callbackTokens),
    );
  }
  ...
  public async planClose(instance: InstanceRecord): Promise<CloseOperation> {
    ...
    return await this.closeStrategy.plan({
      instance,
  ...
  private async cleanupPreviousSurface(
    ...
    if (operation === "replace") {
      await this.ignoreFailure(() =>
        api.deleteMessage(surface.chatId, surface.messageId),
      );
      return;
    }
    if (operation === "send" && surface.hasKeyboard) {
      await this.ignoreFailure(() =>
        api.editMessageReplyMarkup(surface.chatId, surface.messageId, {
          reply_markup: { inline_keyboard: [] },
        }),
      );
    }
  ...
  private async rollbackEdit(
    ...
    try {
      ...
      if (!telegramApplied) {
        await this.ignoreFailure(() =>
          this.repository.deleteCallbacks(rollback.callbackTokens),
        );
      }
  ...
  private async send(
    ...
    switch (rendered.media.kind) {
      case "photo":
        return api.sendPhoto(instance.chatId, rendered.media.source, options);
      case "video":
        return api.sendVideo(instance.chatId, rendered.media.source, options);
      case "animation":
        return api.sendAnimation(
          instance.chatId,
          rendered.media.source,
          options,
        );
      case "audio":
        return api.sendAudio(instance.chatId, rendered.media.source, options);
      case "document":
      ...
      case "voice":
        return api.sendVoice(instance.chatId, rendered.media.source, options);
    }
  }
  ...
  private async edit(
    ...
    if (
      surface.kind !== "text" &&
      surface.kind !== "voice" &&
      rendered.media !== undefined &&
      rendered.media.kind !== "voice"
    ) {
    ...
    throw new Error(
      `Cannot edit '${surface.kind}' surface with a different media kind`,
    );
  }
  ...
  private threadOptions(instance: InstanceRecord): {
    message_thread_id?: number;
  } {
    return instance.threadId === undefined
      ? {}

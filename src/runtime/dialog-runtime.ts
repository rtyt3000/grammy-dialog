 ...
 */
export class DialogRuntime<C extends Context = Context, Services = unknown> {
  ...
  public constructor(options: DialogRuntimeOptions<C, Services>) {
    const storage =
      options.storage ?? new MemoryStorageAdapter<DialogStorageRecord>();
    this.identities =
      options.identities ?? this.storageIdentityCoordinator(storage);
    ...
    this.transitions = new InstanceTransitions(
      this.registry,
      options.maxStackDepth ?? 32,
    );
    this.services = options.services as Services;
    this.codec =
      "encode" in (options.callbacks ?? {})
        ? (options.callbacks as CallbackCodec)
        : createCallbackCodec(
            options.callbacks as CallbackCodecOptions | undefined,
          );
    ...
    this.inputRouting =
      options.defaults?.inputRouting ?? inputRouting.replyOrFocused<C>();
  }
  ...
  public controller(ctx: C): DialogController {
    return {
      ...
      setLocale: (instanceId, locale) =>
        this.setLocale(ctx.api, instanceId, locale, ctx),
    };
  }
  ...
  public async start(
    ...
    if (ctx.chat === undefined)
      throw new Error("Cannot start a dialog without a chat");
    const dialog = this.registry.dialog(dialogReference);
    const initial = this.registry.window(
      this.registry.resolveDialogWindow(dialog, dialog.initial),
    );
  ...
  public async show(
    ...
    if (options.api === undefined)
      throw new Error("ui.show requires an Api instance");
    if (options.chatId === undefined)
      throw new Error("ui.show requires a chatId");
    const selectedWindow = this.registry.window(windowReference);
    const locale =
      options.locale ??
      (ctx === undefined ? this.defaultLocale : await this.resolveLocale(ctx));
    const instance = this.createInstance({
      kind: "standalone",
  ...
  public async handleInput(ctx: C): Promise<boolean> {
    ...
    const candidates = (
      ...
      .filter(
        (instance): instance is InstanceRecord =>
          instance !== undefined && instance.status === "active",
      )
      .map((instance) => ({ id: instance.id, instance }));
    const focusedInstanceId = await this.inputRouting.route({
      ctx,
      candidates,
    });
    if (focusedInstanceId === undefined) return false;
    if (!candidates.some((candidate) => candidate.id === focusedInstanceId))
      return false;
    ...
    return this.locks.run(focusedInstanceId, async () => {
      ...
      const matched = await this.inputs.match(
        ctx,
        await this.renderer.resolveInputs(instance, selectedWindow, ctx),
      );
      ...
      if (matched.failure !== undefined) {
        ...
        await ctx.reply(
          await this.renderer.resolveText(matched.failure, renderContext),
        );
        return true;
      }
      ...
      if (active !== undefined && mode === "reuse") {
        await this.locks.run(active.id, async () => {
          ...
          if (current === undefined || current.status !== "active") {
            throw new Error(
              `Keyed instance '${active.id}' became inactive during reuse`,
            );
          }
          const selectedWindow = this.registry.currentWindow(current);
          if (
            ctx !== undefined &&
            !(await this.canAccess(ctx, current, selectedWindow))
          ) {
            throw new Error(
              `Access denied for existing instance '${current.id}'`,
            );
          }
          current.revision += 1;
          await this.focus.commit(
            current,
            ctx?.from?.id ?? instance.ownerId,
  ...
  private async processAction(ctx: C, callback: CallbackRecord): Promise<void> {
    ...
    if (callback.expiresAt !== undefined && callback.expiresAt < Date.now())
      return;
    if (callback.revision !== instance.revision) return;
    if (callback.chatId !== ctx.chat?.id) return;
    if (instance.surface?.messageId !== ctx.callbackQuery?.message?.message_id)
      return;
  ...
  private async finishUpdate(
    ...
    if (instance.status === "closed") {
      ...
      await Promise.allSettled([
        ...
        ...focusedUserIds.map((userId) =>
          this.repository.deleteFocusIfOwned(
            instance.chatId,
            userId,
            instance.id,
            instance.threadId,
  ...
  private storageIdentityCoordinator(
    ...
    return typeof identities === "object" &&
      identities !== null &&
      "run" in identities
  ...
  private messageThreadId(ctx: C): number | undefined {
    return (ctx.msg as { message_thread_id?: number } | undefined)
      ?.message_thread_id;
  }
  ...
  private async canAccess(
    ...
    const dialog =
      instance.kind === "dialog"
        ? this.registry.findDialog(instance.definitionId)
        : undefined;
    const strategy =
      selectedWindow.access ?? dialog?.access ?? this.defaultAccess;

...
import type { InputDefinition, TextSource } from "../core.js";
...
export class InputMatcher<C extends Context = Context> {
  ...
  private async matchOne(
    ...
    if (input.kind === "text" && ctx.message?.text !== undefined) {
      ...
      const validation = (await input.validate?.(value)) ?? {
        ok: true as const,
        value,
      };
      return validation.ok
        ...
        : {
            ...
            failure: validation.message ?? "Invalid value",
          };
    }
    ...
    if (input.kind === "document" && ctx.message?.document !== undefined) {
      return {
        intent: input.onReceive,
        value: this.file(ctx, ctx.message.document),
      };
    }
